...
class Worker
{
	...
	public function work($interval = 5)
	{
		...
		while(true) {
			...
			$this->log('got ' . $job);
			\Resque\Event::trigger('beforeFork', $job);
			$this->workingOn($job);
	 ...
	 */
	public function perform(\Resque\Job $job)
	{
		try {
			\Resque\Event::trigger('afterFork', $job);
			$job->perform();
		}
		catch(\Exception $e) {
	...
	private function startup()
	{
		...
		\Resque\Event::trigger('beforeFirstFork', $this);
		$this->registerWorker();
	}
	...
	public function unregisterWorker()
	{
		...
		\Resque\Stat::clear('processed:' . $id);
		\Resque\Stat::clear('failed:' . $id);
	}
	 ...
	 */
	public function workingOn(\Resque\Job $job)
	{
		$job->worker = $this;
		$this->currentJob = $job;
	...
	public function doneWorking()
	{
		...
		\Resque\Stat::incr('processed');
		\Resque\Stat::incr('processed:' . (string)$this);
